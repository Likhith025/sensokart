...
const WhatsAppButton = () => {
  const phoneNumber = '917601004575'; // Use without + prefix
  const message = 'Hello! I would like to get more information about your products.';

  const handleWhatsAppClick = () => {
    const encodedMessage = encodeURIComponent(message);
    
    // Method 1: Direct number (recommended)
    const whatsappUrl = `https://wa.me/${phoneNumber}?text=${encodedMessage}`;
    
    // Method 2: If you want to use the + format
    // const whatsappUrl = `https://api.whatsapp.com/send/?phone=${phoneNumber}&text=${encodedMessage}`;
    
    console.log('Redirecting to:', whatsappUrl); // Check console to verify
